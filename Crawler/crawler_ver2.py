...
import time
import csv
from crawler.constant import area_k_to_e, NULL, set_csvfile_name
...
def naver_crawler(area_kor, store_type_kor):
    print("크롤링 시작")
    start_time =  time.time()
    area_eng = area_k_to_e[area_kor]
    ...
    main_url = f'https://map.naver.com/v5/search/{area_kor}%20{store_type_kor}/place'
    ...
    for page in range(1,2):
        ...
        #! 프레임 이동
        searchIframe = crawler.find_element(By.CSS_SELECTOR, '#searchIframe')
        crawler.switch_to.frame(searchIframe)
        crawler.implicitly_wait(2) #? 로딩이 끝날동안 기다리기
        time.sleep(1)

        #? page 클릭하여 이동
        crawler.find_element(By.CSS_SELECTOR, f"div > div.XUrfU > div.zRM9F > a:nth-child({page})").click()

        #! 스크롤 가능하도록 body 중 아무 동작 없는 곳 클릭
        crawler.find_element(By.CSS_SELECTOR, "#_pcmap_list_scroll_container > ul > li > div.CHC5F").click() 
        ...
        shops = crawler.find_elements(By.CSS_SELECTOR, '#_pcmap_list_scroll_container > ul > li')
        #? 가게들 정보 크롤링 시작
        for shop in shops:
            name, type, star_rating, review_sum, address, time_info, contact = \
            ...
            #! 가게 명
            try:
                name = shop.find_element(By.CSS_SELECTOR, 'div.CHC5F > a > div > div > span.place_bluelink.TYaxT').text
            except:
                name = NULL
            crawler.implicitly_wait(2)
            print(name)
            ...
            #! 가게 종류
            try:
                type = shop.find_element(By.CSS_SELECTOR, 'div.CHC5F > a > div > div > span.KCMnt').text
            except:
                type = NULL
            crawler.implicitly_wait(2)
            print(type)
            ...
            #! 가게명 클릭하여 세부창 띄우기
            shop.find_element(By.CSS_SELECTOR, 'div.CHC5F > a > div > div').click()
            crawler.implicitly_wait(2)
        
            #? frame 밖으로 나가기
            crawler.switch_to.default_content()
            #! entryIframe 찾아 들어오기
            entryIframe = crawler.find_element(By.CSS_SELECTOR, '#entryIframe')
            ...
            #! 영업시간 펼쳐보기 클릭 먼저 해놓기!
            try:
                crawler.find_element(By.CSS_SELECTOR, '#app-root > div > div.BXtr_.tAvTy > div > div:nth-child(6) > div > div.place_section.no_margin.vKA6F > div.place_section_content > div.PIbes > div.O8qbU.pSavy > div > a').click()
                print('영업시간 펼쳐보기 element 찾음')
            except:
                print('영업시간 펼쳐보기 element 못찾음')
                error_count += 1
            count += 1
    ...
    with open(set_csvfile_name(area_kor, store_type_kor, 'before'), 'w', encoding= 'UTF-8') as file:
